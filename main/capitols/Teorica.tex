...
\begin{figure}[h]
    ...
    \label{Figura}
\end{figure}
\footnotetext{El programa està escrit en Python. El propòsit és fer-se una idea de com és un programa, ja que en parlarem durant tot el treball. Després dels coixinets hi ha un comentari explicatiu que no afecta el funcionament.}
...
Pràcticament en l'anàlisi dels algoritmes, ens interessa el comportament asimptòtic, per tant:
\[f \in O(g) \Rightarrow 0 \leq \lim\limits_{n \to \infty} |\frac{f(n)}{g(n)}| < \infty \]

% \newpage
% Podem utilitzar la definició 1.2 en programació i anàlisis algorítmic perquè només ens interessa analitzar asimptòticament les complexitats, tot i que la definició 1.1 és la definició matemàtica d'aquesta notació. Podríem dir que la definició 1.2 és un cas particular de la definició 1.1 en què $x_0 \to \infty$.

% Per tant:
% \[f \in O(g) \iff \exists c > 0 / 0 \leq \lim\limits_{n \to \infty} |\frac{f(n)}{g(n)}| \leq c \iff f = O(g) \]
...
Per tant, si hem d'utilitzar un algoritme de complexitat constant per a $n = 1.000.000$, farà la mateixa quantitat d'operacions que per a $n = 10$. I la velocitat d'aquestes operacions dependrà completament de factors externs a l'algoritme (ordinador, implementació, compilador, llenguatge de programació, etc.).
...
\begin{figure}[H]
    \centering
    \begin{tikzpicture}
    \begin{axis}[xmin=0, xmax=10, ymin=0, ymax=50, axis lines = middle,
    y label style={at={(axis description cs:-0.1,.5)},rotate=90,anchor=south},
    xlabel={$n$ mida de l'entrada},
    ylabel={nombre d'operacions},
...
major grid style={line width=.2pt,draw=gray!50},
    ...
    \addplot[color=black, samples=200]{10^x};
\legend{$O(1)$,$O(log_{2}n)$, $O(n)$, $O(n^2)$, $O(e^n)$}
    \end{axis}
    \end{tikzpicture}
...
\subsection{Anàlisi d'algoritmes en estructures de dades no lineals}
...
He fet un vídeo que mostra el funcionament del programa, es pot trobar en el següent enllaç: \url{https://drive.google.com/file/d/1wqpY9PLo06W36MiOt74nirxrB-yl8TjB/view?usp=sharing} o escanejant el codi qr de la figura 1.13.
\begin{figure}[H]
    \centering
